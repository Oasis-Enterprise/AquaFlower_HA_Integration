...
async def async_setup_devices(hass: HomeAssistant, entry: ConfigEntry) -> list:
    """Fetch devices from API and register them in device registry."""
    ...
    try:
        async with session.get(
            ...
            if response.status != 200:
                _LOGGER.error(f"Failed to fetch devices. HTTP Status: {response.status}")
                return []
            
            devices = await response.json()
            if not isinstance(devices, list):
                _LOGGER.error("Invalid device data format")
                return []
            
            _LOGGER.info(f"Fetched {len(devices)} device(s) from AquaFlower API")
            ...
            for device in devices:
                ...
                device_name = device.get("name", f"AquaFlower {device_id}")
                
                device_registry.async_get_or_create(
                    config_entry_id=entry.entry_id,
                    identifiers={(DOMAIN, device_id)},
                    manufacturer="AquaFlower",
                    name=device_name,
                    model="Smart Irrigation Controller",
                )
                _LOGGER.info(f"Registered device: {device_name} ({device_id})")
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    devices = await async_setup_devices(hass, entry)
    ...
    hass.data[DOMAIN][entry.entry_id] = {
        ...
        "devices": devices,  # Store devices for platforms to access
    }
    ...
    # ✅ Register custom Lovelace card static path
    from homeassistant.components.http import StaticPathConfig
    await hass.http.async_register_static_paths([
        StaticPathConfig(
            "/aquaflower_card",
            hass.config.path("custom_components/aquaflower/www"),
            cache_headers=False,
        )
    ])
    _LOGGER.info("Registered AquaFlower custom card static path")
